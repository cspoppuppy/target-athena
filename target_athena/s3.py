...
# LOGGER = singer_sdk.get_logger("target_athena")
...
def create_client(config):
    print("Attempting to create AWS session")
...
@retry_pattern()
def upload_file(
    filename, s3_client, bucket, s3_key, encryption_type=None, encryption_key=None
):
    ...
    if encryption_type is None or encryption_type.lower() == "none":
        ...
        if encryption_type.lower() == "kms":
            ...
            if encryption_key:
                encryption_desc = " using KMS encryption key ID '{}'".format(
                    encryption_key
                )
            ...
            raise NotImplementedError(
                ...
                "Expected: 'none' or 'KMS'".format(encryption_type)
            )
    print(
        "Uploading {} to bucket {} at {}{}".format(
            filename, bucket, s3_key, encryption_desc
        )
    )
